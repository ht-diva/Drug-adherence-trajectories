...
fe = fread("N:/output/data/acorbetta/STATINS/ADHERENCE/ADH_STATINS_ambulatoriale_data_all.csv.gz")

ID = fe$COD_SOGGETTO


fwrite(list(colnames(fe)[-1]),"N:/output/data/acorbetta/STATINS/ADHERENCE/summary_stats/covariates_visits_all.csv")

#isolate labs
LABS = fe$`LABORATORIO ANALISI`

# all else
fe = fe %>%
  select(-`LABORATORIO ANALISI`, -COD_SOGGETTO) %>%
  mutate_all(function(x)fifelse(x>0,1,0))
...
fwrite(fe_p,"N:/output/data/acorbetta/STATINS/ADHERENCE/summary_stats/covariates_visits_prevalence_selection_all.csv")
...
diag(phis_dist) = 1
fwrite(phis_dist,"N:/output/data/acorbetta/STATINS/ADHERENCE/summary_stats/covariates_visits_distance_matrix_all.csv")
...
svg("N:/output/data/acorbetta/STATINS/ADHERENCE/plots/visits_trees_all.svg", width = 12, height = 6)
par(mfrow=c(1,3))
plot(C1,main="Complete Linkage", xlab="", sub="", cex=.9)
plot(C2, main="Single Linkage", xlab="", sub="", cex=.9)
plot((C3),main="Average Linkage", xlab="", sub="", cex=.9)
...
svg("N:/output/data/acorbetta/STATINS/ADHERENCE/plots/visits_selection_all.svg", width = 12, height = 6)
dis_plot
dev.off()
...
fe_wide_s = ID %>% cbind(LABS) %>% cbind(fe_wide[,names(fe_wide) %in% fe_ps$VAR])
colnames(fe_wide_s)[1] = "COD_SOGGETTO"
colnames(fe_wide_s)[2] = "LABORATORIO"

colnames(fe_wide_s) = gsub(" ","_",colnames(fe_wide_s))

fwrite(fe_wide_s,"N:/output/data/acorbetta/STATINS/ADHERENCE/visits_covariates_all.csv")
