...
load("/media/volume/mferro/data/ADHERENCE/working_prediction.rds")


#### READ FILE ####
cov = fread("/media/volume/mferro/data/ADHERENCE/ALL_COVARIATES_MPR.tsv")
cov[,T_GROUPS := as.factor(fifelse(T_GROUPS == "High", 0 ,1))]
cov[,zip_code := as.factor(zip_code)]
cov[,urban_rural_class_code := as.factor(urban_rural_class_code)]
cov[,FINREGISTRYID := NULL]
...
recipe <- recipe(T_GROUPS ~ ., data = train_data) %>%
  step_dummy(zip_code,urban_rural_class_code) %>%
...
# Extract the model coefficients and p-values using `tidy()`
coef_df <- tidy(logistic_fit, exponentiate = T, conf.int = T) %>%
...
fwrite(coef_df,"/media/volume/mferro/summary_stats/coefficients_logistic.csv")
#coef_df = fread("/media/volume/mferro/summary_stats/coefficients_logistic.csv")
...
svg("/media/volume/mferro/plots/logistic_volcano_MAIN.svg", width = 10, height = 10)
volc_plot
dev.off()
...
fwrite(metr,"/media/volume/mferro/summary_stats/metrics_lasso.csv")
...
fwrite(coef_lasso,"/media/volume/mferro/summary_stats/coef_lasso.csv")
coef_lasso = fread("/media/volume/mferro/summary_stats/coef_lasso.csv")
setDT(coef_lasso)
...
svg("/media/volume/mferro/plots/lasso_coefs_MAIN.svg", width = 8, height = 12)
imp_lasso_plot
dev.off()
...
metr_lgb = lightgbm_tune %>%
  collect_metrics()
fwrite(metr_lgb,"/media/volume/mferro/summary_stats/metrics_lgb.csv")
...
save.image("/media/volume/mferro/data/ADHERENCE/working_prediction.rds")
...
fwrite(importance_matrix,"/media/volume/mferro/summary_stats/imp_lightgbm.csv")
importance_matrix = fread("/media/volume/mferro/summary_stats/imp_lightgbm.csv")
imp_lgb_plot = importance_matrix[Gain>=0.01] %>%
  ...
  labs(y = NULL) +
  theme_minimal() 
svg("/media/volume/mferro/plots/lgb_import_MAIN.svg", width = 8, height = 10)
imp_lgb_plot
dev.off()
...
fwrite(combined_roc_data,"/media/volume/mferro/summary_stats/performances_roc.csv")
fwrite(combined_performances,"/media/volume/mferro/summary_stats/performances_summary.csv")
...
svg("/media/volume/mferro/plots/ROC_CURVES_MAIN.svg", width = 10, height = 10)
roc
dev.off()
